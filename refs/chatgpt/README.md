...
7. **创建定时任务**：打开 [`weekly-review.md`](scheduled-task-prompt/weekly-review.md)，复制原始 Markdown 并粘贴到刚才的专用聊天，将 `YOUR_GITHUB_USERNAME/YOUR_REPOSITORY` 替换为自己的仓库，再设置执行时间。后续结果会回到同一聊天，方便继续回答问题和发展文章。`break-bubble.md` 也使用同样的方法。
