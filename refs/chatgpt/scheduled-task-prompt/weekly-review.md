...
无论本周是否存在写作方向，都把结果保存到 `reviews/{YYYY}/{YYYYMM}/{开始日期}-{结束日期}-{关键词}.md`。关键词应简短、具体且来自本周材料；同一日期范围已有 Review 时更新原文件并保留原文件名，不创建重复版本。保存成功后，将回看摘要、问题、可能发展的 0–2 个方向和 Review 路径返回当前聊天。

定时运行时不要直接生成文章或创建 Practice。等我在当前聊天中回答问题或明确确认某个方向后，再继续执行 Skill 的后续回应或可选文章分支。
