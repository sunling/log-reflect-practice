...
AI 对话环境中的本地文件通常是临时的，完成记录必须通过 GitHub API（或已挂载的 GitHub 插件）写入配置的目标仓库。`discover` 阶段只推荐资源，不创建文件；只有进入 `complete` 后才执行 GitHub 写入。
...
- 配置后的完整仓库名是唯一事实来源。每次 GitHub 调用都必须使用它，不得从资源链接、模板作者、历史对话或其他项目推断仓库，也不得在失败后改投另一个仓库。
- 不要假定默认分支叫 `main`。优先省略 `ref` / `branch`；工具要求显式分支时，先读取仓库信息中的 `default_branch`，后续所有读写使用同一分支。
- `complete` 本轮第一次访问 GitHub 时，先验证仓库完整名称、可访问性与默认分支。仓库或分支返回 `404` 时停止，不得将其视为“完成记录还不存在”。

### 保存完成记录

1. 按 `daily/inputs/{YYYY}/{YYYYMM}/{YYYYMMDD}-{关键词}.md` 确定路径。
2. 在已验证的仓库和默认分支中检查同一资源是否已有完成记录。
3. 文件不存在时创建；已存在时完整读取原文与当前 SHA，只追加真实的新完成时间或用户本轮主动提供的想法。
4. 新建使用类似 `feat: add daily input {关键词}`，更新使用类似 `docs: update daily input {关键词}`。
5. 只有写入接口明确成功，并返回 commit SHA、commit URL 或等价成功结果后，才可以说“已成功提交”；若结果含糊，重新读取目标路径确认。

### 错误处理与真实反馈

- 只有仓库与默认分支已验证成功后，目标文件或年月目录的 `404` 才能解释为尚不存在。
...
- 当前环境不能写入时，返回目标路径和完整待写入 Markdown，明确说明需要手动保存或补充权限。
- 任何失败都不得提交到模板仓库、作者仓库或历史对话中的其他仓库。

成功时简洁返回仓库、相对路径与 commit 信息；失败时必须明确使用“未提交”或“尚未写入”。
