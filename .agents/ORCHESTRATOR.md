...
如果回看中发现可能的 Practice，先展示证据并征得使用者确认，再创建目录。如果发现可能的输出方向，只提供少量候选；使用者选择一个主题并确认继续后，再交给 `develop-article`。需要先把表达想清楚时进入 Grill Me 模式。文章应归入一个已经立项的输出练习，首稿保存到 `practices/<输出练习名>/drafts/YYYYMMDD-文章主题.md`，后续继续更新同一文件；不要把单篇文章主题直接当作 Practice 名称。
